...
StudySpark is a mobile-first web app built around one loop: you tell it what you want to learn, and it teaches you. Type a topic — "basic derivatives," "how to swing a golf club," "who was Nikola Tesla" — and StudySpark writes you a roadmap of small lessons, ordered so each one builds on the last. Each lesson is what I call a **micro-lecture**: a short, plain-language explanation — 300 to 500 words — of one specific piece of the topic, generated on the spot rather than pulled from a fixed textbook. You read the micro-lecture, take a short quiz on it, and watch your progress, streak, and points fill in against other learners on a leaderboard. Every piece of that content — the roadmap, the lecture, the quiz — is written live by a large language model, not authored in advance.
...
It's worth being explicit about something that could otherwise stay implicit: there's more than one reasonable way to generate a course's worth of teaching content. A Wikipedia-style model — a shared, growing bank of lessons that improves as people contribute to it — was a real option. So was a curated model, where a person or team writes and vets every lesson ahead of time. I chose a third path: an LLM writes the lesson live, the moment a learner asks for it. That choice follows directly from the first research finding above. If a student has to wait on a lesson to be written, reviewed, or approved, they've already lost the moment of interest that got them to open the app in the first place. StudySpark had to answer "I want to learn this" with something *now*, not next week. Near-instant generation is what an LLM uniquely offers here, and it's the reason the rest of this report exists — nearly everything difficult about this project follows from choosing speed over the safety of pre-written content.
